<%@ Page Title="" Language="C#" MasterPageFile="~/SideModulesDefault.master" AutoEventWireup="true" CodeFile="NewOrder.aspx.cs" Inherits="Customer_NewOrder" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">
    <title>Ktrade - оформление заказа</title>
    <link rel="Stylesheet" href="../css/Orders.css" />
    <link rel="Stylesheet" href="../css/Offers.css" />
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    <asp:SqlDataSource ID="CatalogueListSource" runat="server" ConnectionString="<%$ ConnectionStrings:KtradeConnectionString %>"
        SelectCommand="SELECT [Catalogue_id], [CatalogueName] FROM [Catalogues] where [Active] = 1 ORDER BY [CatalogueName]">
    </asp:SqlDataSource>
    <asp:SqlDataSource ID="OrderDetailsSource" runat="server" ConnectionString="<%$ ConnectionStrings:KtradeConnectionString %>"
        SelectCommand="
            select
                OI.id,
                OI.Catalogue_id,
    ...
    <asp:SqlDataSource ID="UpsellOffersSource" runat="server" ConnectionString="<%$ ConnectionStrings:KtradeConnectionString %>"
        SelectCommand="
            select top 3
                ...
                C.CatalogueName,
                'http://ktrade.kz/Customer/NewOrder.aspx?catalogue=' + convert (nvarchar(5), UO.Catalogue_id) + '&article=' + convert(nvarchar(25), Article_id) + '&name=' + convert(nvarchar(25), ArticleNameDe) + '&price=' + replace(convert(nvarchar(25), PriceNew), '.', ',') + '&URL=' + replace (CatalogueURL, 'http://', '') as OrderURL
            from
    ...
    <h2>Новый заказ</h2>
    <p>Здесь вы можете оформить заказ. Если у вас вознинукт затруднения с оформлением заказа, у нас есть <a href="../Infos/OrderMaking.aspx">инструкция</a>, где все доступно и подробно описано.</p>
    ...
    <div class="inputfield">
        ...
        <asp:CustomValidator ID="MinimumPriceValidator" runat="server" ControlToValidate="PriceInput" Display="None" ErrorMessage="Цена артикула должна быть не менее 5 евро." ValidationGroup="OrderPositionValidationGroup">*</asp:CustomValidator>
    </div>
    <div class="inputfield">
