...
public partial class Customer_NewOrder : System.Web.UI.Page
{
    ...
    static string OrderAmmountPaymentString = "select convert (varchar(10),OrderAmmount) as OrderAmmount from OrderList where order_id = @Order_id";
	...
	try
	{
	    Preis = Single.Parse(PriceInput.Text);
	}
	catch
	{
	    Preis = 0;
	}
	if (Preis < GetMinPrice(Int16.Parse(CatalogueList.SelectedValue)))
        {
        ...
        if (Page.IsValid)
        {
            ...
            AddItem.Parameters.AddWithValue("Price", Preis);
    ...
    protected void OrderDetailsGridView_DataBound(object sender, EventArgs e)
    {
	    ...
	    EmptyOrderLabel.Text = "Заказ пуст";
        }
        else
        {
            //EmptyOrderLabel.Visible = true;
	...
	SqlParameter SessionID = new SqlParameter("@Session_id", SqlDbType.NVarChar);
        ...
        Response.Redirect("~/Customer/OrderThanks.aspx?ammount=" + GetOrderAmmountPayment(Order.Value.ToString()));
    }
    ...
    public float GetMinPrice (int Catalogue_id)
    {
	CatalogueInfo.Parameters.Clear();
	CatalogueInfo.Parameters.AddWithValue("Catalogue_id", Catalogue_id.ToString());
	SqlDataReader CatalogueTerms = CatalogueInfo.ExecuteReader();
	CatalogueTerms.Read();
	float MP = Single.Parse(CatalogueTerms["MinPrice"].ToString());
	CatalogueTerms.Close();
	return MP;
    }
