...
public partial class Admin_OrderManagement : System.Web.UI.Page
{
    ...
    static string EcomPaymentDataString = "select top 1 convert (nvarchar (10), EComChargePayments.PaymentTime, 104) as PaymentDate , EComChargePayments.Ammount, isnull(C.FirstName, '-') as FirstName, isnull (OMD.CustomerFullName, C.FirstName + ' ' + C.LastName) as FullName, isnull (OMD.CustomerEmail, M.Email) as Email from EComChargePayments left join Customers as C on EComChargePayments.Customer_id = C.Customer_id left join aspnet_Membership as M on M.UserId = C.User_id left JOIN OrdersMetaData as OMD on OMD.Order_id = EComChargePayments.Order_id where EComChargePayments.transaction_uid = @transaction_uid";
